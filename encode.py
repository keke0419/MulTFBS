...
#  total=pow(alphabet_len,k)
  ...
  for k_ind in range(k):
    ...
    kmersNum=0
    for pop_ind in range(poptotal):
      oldseq=kmersList.pop(0)
      for char_ind in range(alphabet_len):
...
def loadKmer(inputfilename, k=None):
  ...
  kmersDict={}
  for kmer_ind in range(len(kmersList)):
...
def loadUniqKmer(inputfilename, k=None):
  ...
  if k is not None:
    assert k==len(list(uniqKmersDict.keys())[0]), "paremeter k=%d needs to be match sequences in inputfile." % k
  else:
    k=len(uniqKmer)
...
def calculateDNAShape(seqs, seqlen, seqNum, shapeDict):
    ...
    for index in range(seqNum):
        seq= seqs[index]
        RollP = 0.0
        HelTP = 0.0
        for seq_index in range(seqlen-4):
...
def buildKmerFeature(trainSeqs, k, alphabet=None,
  ...
  for seq_ind in range(nRows):
    for seq_kmer_start in range(len(trainSeqs[0])-k+1):
...
def convert_shape(seqs, intensity, seqs_flank, mapper, shapeDict, out_path, name):
    ...
    for seq in seqs:
       mat = []
       for element in seq:
           if element in list(mapper.keys()): mat.append(mapper[element])
           else: print('invalid character!', file=sys.stderr); sys.exit(1)
...
if __name__ == "__main__":
    ...
    if args.shapefile == "":
       print("Need shape file", file=sys.stderr);sys.exit(1)
    else:
